...
const renderInstructions = function(widget) {
	...
	if (widget.tabs.length >= 3) {
		...
		dragDownTextDiv.style.setProperty(
			'left',
			widget.getTabSize() * 2 + 'px'
		);
	} else {
	...
	resizeLeftTextDiv.style.setProperty(
		'top',
		widget.contentHeight / 2 + 10 + 'px'
	);

	resizeRightImg.style.setProperty(
		'top',
		(widget.contentHeight * 2) / 3 + 'px'
	);
	resizeRightTextDiv.style.setProperty(
		'top',
		(widget.contentHeight * 2) / 3 + 10 + 'px'
	);
};
